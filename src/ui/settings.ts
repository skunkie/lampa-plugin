...
export class SettingsUi {
  public static init(): void {
    ...
    this.updateTorrServerVisibility();
    this.updateAdShieldVisibility();

    if (typeof Lampa !== 'undefined' && Lampa.Storage?.listener) {
      Lampa.Storage.listener.follow('change', (event: LampaStorageChangeEvent) => {
        if (event.name === PLAYBACK_MODE_STORAGE_KEY || event.name === TORRPLAY_ENABLED_STORAGE_KEY) {
          this.updateTorrServerVisibility();
        }
        if (event.name === TORRPLAY_ENABLED_STORAGE_KEY) {
          this.updateAdShieldVisibility();
        }
      });
    }
  ...
  public static updateAdShieldVisibility(): void {
    if (typeof Lampa === 'undefined' || !Lampa.Storage) return;
    const shouldShield = Lampa.Storage.get<boolean>(TORRPLAY_ENABLED_STORAGE_KEY, true);

    if (typeof document !== 'undefined') {
      const styleId = 'torrplay-ad-shield-style';
