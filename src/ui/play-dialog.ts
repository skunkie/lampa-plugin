  ...
  public static async resolveOptions(title?: string): Promise<PlayOptions | null> {
    ...
    return new Promise<PlayOptions | null>(resolve => {
      ...
      const rootElement = $(`
        <div class="torrplay-options-modal" style="padding: 1.5em; font-size: 1.1em;">
          ...
          <div style="margin-bottom: 1.5em;">
            <label class="torrplay-opt-remember selector" style="display: flex; align-items: center; cursor: pointer; color: #ccc;">
              <input type="checkbox" style="margin-right: 0.6em; width: 1.2em; height: 1.2em;" />
              ${rememberText}
            </label>
          </div>
      ...
      const updateStorageAvailability = () => {
        ...
        if (shouldSaveToDatabase) {
          storageButton.removeClass('torrplay-opt-disabled').css({ cursor: 'pointer', opacity: 1 });
          storageHint.css('display', 'none');
        } else {
          storageButton.addClass('torrplay-opt-disabled').css({ cursor: 'default', opacity: 0.4 });
      ...
      rememberChoiceCheckbox.on('change', () => {
        shouldRememberChoice = rememberChoiceCheckbox.prop('checked');
      });
