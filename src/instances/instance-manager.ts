...
export class InstanceManager {
  ...
  public static load(): TorrPlayInstance[] {
    ...
    try {
      ...
      if (Array.isArray(rawInstances)) {
        ...
        this.instances = (rawInstances as Array<TorrPlayInstance & { isEnabled?: boolean }>).map(
          ({
            isEnabled: _unused,
  ...
  public static updatePrimaryInstance(fields: Partial<TorrPlayInstance>): void {
    const primary = this.getPrimaryInstance();
    if (primary) {
      Object.assign(primary, fields);
      this.updateInstance(primary);
    }
  }
