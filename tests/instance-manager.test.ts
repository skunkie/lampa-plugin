...
describe('InstanceManager', () => {
  ...
  it('announces every change to which instance requests are routed to', () => {
    const notifications: string[] = [];
    InstanceManager.onSelectionChanged(() => notifications.push('changed'));

    InstanceManager.addInstance({ authType: 'none', id: 'inst-a', name: 'A', url: 'http://a.example' });
    InstanceManager.addInstance({ authType: 'none', id: 'inst-b', name: 'B', url: 'http://b.example' });

    InstanceManager.setSelectedId('inst-a');
    assert.equal(notifications.length, 1, 'Selecting a different instance must announce the change');

    InstanceManager.setSelectedId('inst-a');
    assert.equal(notifications.length, 1, 'Re-selecting the same instance changes nothing to announce');

    InstanceManager.setMode('manual');
    assert.equal(notifications.length, 2, 'Switching selection mode reroutes requests');

    InstanceManager.setMode('manual');
    assert.equal(notifications.length, 2, 'Re-applying the same mode changes nothing to announce');

    InstanceManager.removeInstance('inst-b');
    assert.equal(notifications.length, 3, 'Removing an instance reshapes the pool requests are drawn from');

    (InstanceManager as any).selectionListeners.length = 0;
  });
