...
import { extractHashFromMagnet, normalizeInfoHash, TorrPlayApi, TorrPlayApiError } from '../src/api/torrplay';
...
describe('TorrPlayApi', () => {
  ...
  it('constructs stream URLs with magnet query parameter', async () => {
    ...
    const magnetUri = 'magnet:?xt=urn:btih:0123456789abcdef&dn=Test';
    ...
    assert.ok(streamUrl.includes(`&magnet=${encodeURIComponent(magnetUri)}`));
  });
