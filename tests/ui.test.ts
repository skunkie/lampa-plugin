...
describe('UI & Settings Integration', () => {
  ...
  it('renders torrent cards matching native Lampa Torrents card styling', () => {
    ...
    const sampleTorrents = [
      {
        files: [{ index: 0, length: 1048576, name: 'movie.mkv', path: 'movie.mkv' }],
        hash: 'abc123456789',
      ...
      {
        files: [
          { index: 0, length: 1048576, name: 'video-1.mp4', path: 'video-1.mp4' },
          { index: 1, length: 1048576, name: 'video-2.mp4', path: 'video-2.mp4' },
        ],
        hash: 'def987654321',
      ...
      {
        files: [
          { index: 0, length: 1048576, name: 'part-1.mkv', path: 'part-1.mkv' },
          { index: 1, length: 1048576, name: 'part-2.mkv', path: 'part-2.mkv' },
          { index: 2, length: 1048576, name: 'part-3.mkv', path: 'part-3.mkv' },
        ],
        hash: 'unknown123456',
  ...
  it('strips HTML tags from torrent and file titles sourced from tracker metadata', () => {
    const createMockElement = (tag: any) => {
      if (tag && typeof tag === 'object') {
        return tag;
      }
    ...
    component.renderTorrents([
      {
        files: [{ index: 0, length: 1048576, name: '<img src=x onerror=alert(1)>episode.mkv', path: 'episode.mkv' }],
        hash: 'hash1',
        name: 'Fallback Name',
        storage: 'memory',
    ...
    const multiFileTorrent: any = {
      files: [
        { index: 0, length: 1048576, name: '<img src=x onerror=alert(1)>episode-1.mkv', path: 'episode-1.mkv' },
        { index: 1, length: 1048576, name: 'episode-2.mkv', path: 'episode-2.mkv' },
      ],
      hash: 'hash1',
      name: 'Fallback Name',
  ...
  it('opens multi-file torrent via Lampa.Modal with torrent-file styled cards', () => {
    ...
    const multiFileTorrent: any = {
      files: [
        ...
        { index: 2, length: 10240,     name: 'subtitle.srt',       path: 'subtitle.srt' },
      ],
      hash: 'multi123',
      name: 'My Show S01',
      title: 'My Show S01',
    ...
    // File items should use torrent-file selector class
    const fileCards = capturedItems.filter(s => s.includes('torrent-file selector'));
    // Only video files (mkv) should be shown, not subtitle.srt
  ...
  it('shows an error notice and stops loading when playback fails from the database torrents browser', async () => {
    ...
    try {
      ...
      const singleFileTorrent: any = {
        files: [{ index: 0, length: 1000, name: 'movie.mkv', path: 'movie.mkv' }],
        hash: 'single123',
        name: 'Movie',
      };
  ...
  it('manages declarative ad shield stylesheet according to settings', () => {
    ...
    // Enabled by default
    storageMap.set(TORRPLAY_ENABLED_STORAGE_KEY, true);
    SettingsUi.updateAdShieldVisibility();

    const shieldStyle = document.getElementById('torrplay-ad-shield-style');
    assert.ok(shieldStyle);
    assert.ok(shieldStyle.textContent.includes('.ad-preroll'));
    assert.ok(shieldStyle.textContent.includes('.ad-video-block'));
    assert.ok(shieldStyle.textContent.includes('.ad-banner'));
    assert.ok(shieldStyle.textContent.includes('display: none !important'));

    // Calling again does not duplicate stylesheet
    SettingsUi.updateAdShieldVisibility();
    assert.equal(headElements.length, 1);

    // Disabling TorrPlay removes the stylesheet
    storageMap.set(TORRPLAY_ENABLED_STORAGE_KEY, false);
    SettingsUi.updateAdShieldVisibility();
    assert.equal(document.getElementById('torrplay-ad-shield-style'), null);

    // Re-enabling creates the stylesheet again
    storageMap.set(TORRPLAY_ENABLED_STORAGE_KEY, true);
    SettingsUi.updateAdShieldVisibility();
    assert.ok(document.getElementById('torrplay-ad-shield-style'));
  });
});
